...
export class TarefasService {
  ...
  listar():Observable<Tarefas[]>{
    ...
    return collectionData(tarefas, {idField:'nome'}) as Observable<Tarefas[]>;
  }

  buscar(id:any):Observable<Tarefas>{
    const tarefas = doc(this.firestore, this.colecao + '/' + id);

    return docData(tarefas, {idField: 'nome'}) as Observable<Tarefas>;
  }

  alterar(obj: any){
    const tarefas = doc(this.firestore, this.colecao + '/' + obj.id);
